...
describe('Uniswap V2:', () => {
    ...
    before(async () => {
        ...
        await wethContract.connect(alice).deposit({value: expandTo18DecimalsBN(11100)})
        await new Promise(f => setTimeout(f, 5000));
        await wethContract.connect(alice).transfer(bob.address, expandTo18DecimalsBN(11100))
        await daiContract.connect(alice).mint(bob.address, expandTo18DecimalsBN(11000000))
        await usdcContract.connect(alice).mint(bob.address, expandTo6DecimalsBN(100000))
    ...
    describe('UniswapV2 setup', () => {
        ...
        it.skip('swap dai to weth', async () => {
            ...
            await uniswapV2Router.connect(bob).swapExactTokensForTokens(
                amountIn,
                amountOutMin,
                path,
                to,
