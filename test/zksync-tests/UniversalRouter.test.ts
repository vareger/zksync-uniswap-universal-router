import { Permit2, UniversalRouter, ERC20 } from '../../typechain'
import {
    ALICE_PRIVATE_KEY,
    BOB_PRIVATE_KEY,
    MAX_UINT
...
describe('UniversalRouter Test:', () => {
    ...
    beforeEach(async () => {
        ...
        //await erc20.connect(alice).approve(permit2.address, MAX_UINT)
    ...
    it('testSweepToken', async () => {
        await erc20.connect(alice).mint(router.address, expandTo18DecimalsBN(1))
    ...
    it('testSweepTokenInsufficientOutput', async () => {
        ...
        planner = new RoutePlanner();
        planner.addCommand(CommandType.PAY_PORTION, [erc20.address, alice.address, 11_000])
    ...
    it('testSweepERC1155NotFullAmount', async () => {
        await erc1155.connect(alice).mint(router.address, 1, expandTo18DecimalsBN(1))
