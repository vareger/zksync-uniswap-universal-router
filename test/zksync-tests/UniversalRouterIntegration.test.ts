...
describe('UniversalRouter', () => {
  ...
  beforeEach(async () => {
    ...
    delay(3000)
    let pairAddress = await (await uniswapV2Factory.createPair(daiContract.address, wethContract.address)).wait()
    delay(5000)
  ...
  //   })
    ...
    describe('ERC20 --> NFT', () => {
      ...
      beforeEach(async () => {
        value = BigNumber.from('33000000000000000000')
      })

      it('completes a trade for ERC20 --> ETH --> Seaport NFT', async () => {
        const maxAmountIn = expandTo18DecimalsBN(100_000)
        await wethContract.connect(alice).mint(router.address, '1000000000000000000000')
        await daiContract.connect(alice).mint(router.address, expandTo18DecimalsBN(50))
        await daiContract.approve(permit2.address, MAX_UINT)
        await permit2.approve(daiContract.address, router.address, MAX_UINT160, DEADLINE)
        await daiContract.approve(router.address, MAX_UINT)
        await daiContract.approve(permit2.address, MAX_UINT)
        await daiContract.connect(alice).mint(alice.address, expandTo18DecimalsBN(50))
