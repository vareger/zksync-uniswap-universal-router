import {IWETH9, MockERC20, Permit2, UniversalRouter} from '../../typechain'
import {
    ALICE_PRIVATE_KEY,
    BOB_PRIVATE_KEY,
    MAX_UINT,
...
import {UniswapV2Factory, UniswapV2Router02} from './shared/deployUniswapV2'
import {deployUniswapV3, UniswapV3Factory} from './shared/deployUniswapV3'
...
import {v2FactoryFixture, v3RouterFixture} from "@uniswap/v3-periphery/test/shared/externalFixtures";
...
describe('Uniswap V2 and V3 Tests:', () => {
    ...
    let daiContract: MockERC20
    let usdcContract: MockERC20
    let usdtContract: MockERC20
    ...
    before(async () => {
        ...
        const {factory, weth9, router} = await v3RouterFixture([alice])
        // @ts-ignore
        let foo = router;
        ...
        console.log(2);
        daiContract = <MockERC20> await deployer.deploy(MockERC20, [6])
        usdcContract = <MockERC20> await deployer.deploy(MockERC20, [6])
        usdtContract = <MockERC20> await deployer.deploy(MockERC20, [6])
        //wethContract = await wethFixture()
        console.log(4);
        //let uniswapV2 = await deployUniswapV2(wethContract.address)


        //const { factory: factoryV2 } = await v2FactoryFixture([alice]);
        const {factory: factoryV2 } = await v2FactoryFixture([alice])
        ...
        console.log(7);
        await (await wethContract.connect(alice).deposit({value: expandTo18DecimalsBN(1110)})).wait()
        await (await wethContract.connect(alice).transfer(bob.address, expandTo18DecimalsBN(1110))).wait()
    ...
    describe('Trade on UniswapV2', () => {
        ...
        beforeEach(async () => {
            ...
            await (await permit2.approve(daiContract.address, universalRouter.address, MAX_UINT160, DEADLINE)).wait()
            await (await permit2.approve(wethContract.address, universalRouter.address, MAX_UINT160, DEADLINE)).wait()
            planner = new RoutePlanner()
        })
        ...
        describe('ERC20 --> ERC20', () => {
            ...
            it('exactIn trade, where an output fee is taken', async () => {
                ...
                await (await universalRouter['execute(bytes,bytes[],uint256)'](commands, inputs, DEADLINE)).wait()
        ...
        describe('ERC20 --> ETH', () => {
            ...
            it('completes a V2 exactOut swap, with ETH fee', async () => {
                ...
                planner.addCommand(CommandType.UNWRAP_WETH, [universalRouter.address, amountOut])
                ...
                const receipt = await (await universalRouter['execute(bytes,bytes[],uint256)'](commands, inputs, DEADLINE)).wait()
        ...
        describe('ETH --> ERC20', () => {
            ...
            it('completes a V2 exactOut swap', async () => {
                ...
                planner.addCommand(CommandType.WRAP_ETH, [universalRouter.address, value])
                planner.addCommand(CommandType.V2_SWAP_EXACT_OUT, [
                    MSG_SENDER,
                    amountOut,
                    value,
    ...
    describe('Trade on UniswapV3', () => {
        ...
        beforeEach(async () => {
            // for these tests Bob gives the router max approval on permit2
            await permit2.approve(daiContract.address, universalRouter.address, MAX_UINT160, DEADLINE)
            await permit2.approve(wethContract.address, universalRouter.address, MAX_UINT160, DEADLINE)
            planner = new RoutePlanner()
        })
        ...
        const addV3ExactInTrades = (
            ...
            for (let i = 0; ethers.BigNumber.from(i).lt(numTrades); i++) {
        ...
        describe('ERC20 --> ETH', () => {
            it('completes a V3 exactIn swap', async () => {
                const amountOutMin: BigNumber = expandTo18DecimalsBN(0.0005)
                addV3ExactInTrades(planner, 1, amountOutMin, universalRouter.address)
                planner.addCommand(CommandType.UNWRAP_WETH, [MSG_SENDER, 0])
        ...
        describe('ETH --> ERC20', () => {
            it('completes a V3 exactIn swap', async () => {
                ...
                planner.addCommand(CommandType.WRAP_ETH, [universalRouter.address, amountIn])
            ...
            it('completes a V3 exactOut swap', async () => {
                ...
                planner.addCommand(CommandType.WRAP_ETH, [universalRouter.address, amountInMax])
        ...
        describe('with Universal Router.', () => {
            ...
            describe('Split routes', () => {
                ...
                it('ERC20 --> ERC20 split V2 and V3, one hop', async () => {
                    ...
                    planner.addCommand(CommandType.V2_SWAP_EXACT_IN, [universalRouter.address, v2AmountIn, 0, tokens, SOURCE_MSG_SENDER])
                ...
                it('ETH --> ERC20 split V2 and V3, one hop', async () => {
                    ...
                    planner.addCommand(CommandType.WRAP_ETH, [universalRouter.address, value])
                    planner.addCommand(CommandType.V2_SWAP_EXACT_IN, [universalRouter.address, v2AmountIn, 0, tokens, SOURCE_ROUTER])
                    planner.addCommand(CommandType.V3_SWAP_EXACT_IN, [
                        universalRouter.address,
                        v3AmountIn,
                        0,
    ...
    async function executeRouter(planner: RoutePlanner, value?: BigNumberish): Promise<ExecutionParams> {
        ...
        const receipt = await (await universalRouter['execute(bytes,bytes[],uint256)'](commands, inputs, DEADLINE, {value})).wait()
        const gasSpent = receipt.gasUsed.mul(receipt.effectiveGasPrice)
