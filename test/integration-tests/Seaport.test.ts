...
describe('Seaport', () => {
  ...
  beforeEach(async () => {
    ...
    router = (await deployUniversalRouter(permit2, ZERO_ADDRESS, mockSeaport.address)).connect(alice) as UniversalRouter
  ...
  it('revertable fulfillAdvancedOrder reverts and sweeps ETH', async () => {
    ...
    await (
      await mockERC721
        .connect(alice)
        .mint('0x0f1fcc9da5db6753c90fbeb46024c056516fbc17', params.offer[0].identifierOrCriteria)
    ).wait()
