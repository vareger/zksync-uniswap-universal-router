...
import { ALICE_PRIVATE_KEY } from '../integration-tests/shared/constants'
...
describe('UniversalRouter Test:', () => {
  ...
  it('testSweepETH', async () => {
    ...
    planner.addCommand(CommandType.SWEEP, [
      ...
      RECIPIENT,
      expandTo18DecimalsBN(1000000000),
    ])
    const { commands, inputs } = planner
    await (await router['execute(bytes,bytes[])'](commands, inputs)).wait()
  ...
  it('testSweepETHInsufficientOutput', async () => {
    planner.addCommand(CommandType.SWEEP, [
      '0x0000000000000000000000000000000000000000',
      RECIPIENT,
      expandTo18DecimalsBN(1000000001),
    ])
    const { commands, inputs } = planner
    const value = expandTo18DecimalsBN(1000000000)
    await expect(router['execute(bytes,bytes[])'](commands, inputs, { value })).to.be.revertedWithCustomError(
      router,
      'InsufficientETH'
    )
  })
